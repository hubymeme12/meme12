...
class element {
	private:
		...
		// flag for adjusting the target index to search in
		int target;
	public:
		// constructor to initialize values
		element();

		// set flag where searching algorithm will search
		// parameter: char - search flag
		void set_flag(char);

		// display the details using the index of the element
		void interpret_index(int);

		// search for matched strings
		int search(string);

		// input validation
		string search_filterN(string);
		string search_filterA(string);
};
...
void element::set_flag(char flag) {
	if (flag != uflag) {
		uflag = flag;
		switch (flag) {
			case 'n':
...
void element::interpret_index(int x) {
	string box;
	...
	if (x < 0) {
		...
		// quantum numbers (calculate number of orbits)
		int pq = getPrincipal(enumb);

		// interpret the number of orbits
...
int element::search(string elem) {
	for (int i = 0; i < 118; i++) {
